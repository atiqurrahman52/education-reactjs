...
const Modal = ({ setIsModalVisible }) => {
  return (
    <div className="modal fixed z-50 inset-0 flex h-screen w-full items-center justify-center px-4">
      <div
        className="modal-overlay fixed inset-0 bg-black/40 h-screen w-full items-center justify-center z-30"
        onClick={() => setIsModalVisible(false)}
      ></div>
      <div className="modal-content max-w-[1100px] h-auto relative z-40">
        <button
          onClick={() => setIsModalVisible(false)}
          type="button"
        ...
        <video controls src={video} className="w-full h-full rounded-md">
